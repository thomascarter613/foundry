...
FILES: dict[str, str] = {
    "packages/cli/src/docs/adr-validator.ts": '''import { parseMarkdownDocument } from "./frontmatter.js";
...
''',
    ...
    "packages/cli/src/commands/docs/adr/validate.ts": '''import { Command, Flags } from "@oclif/core";
...
def update_package_json() -> None:
    ...
    scripts.setdefault(
        "docs:adr:validate",
        "bun run foundry:build && node packages/cli/bin/run.js docs adr validate"
    )
    scripts.setdefault(
        "docs:adr:validate:strict",
        "bun run foundry:build && node packages/cli/bin/run.js docs adr validate --strict-index --fail-on-warnings"
    )
