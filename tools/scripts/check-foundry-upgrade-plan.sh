...
ARTIFACT_DIR=".artifacts/foundry/upgrade-plan"
SUMMARY_TXT="$ARTIFACT_DIR/summary.txt"
SUMMARY_JSON="$ARTIFACT_DIR/summary.json"
PLAN_JSON=".artifacts/foundry/upgrade/plan.json"

mkdir -p "$ARTIFACT_DIR" ".artifacts/foundry/upgrade"
: > "$SUMMARY_TXT"
...
record "Foundry upgrade plan smoke gate"
...
run_capture "upgrade plan text" "$ARTIFACT_DIR/upgrade-plan-text.log" \
  node packages/cli/bin/run.js upgrade --plan

run_capture "upgrade plan json" "$ARTIFACT_DIR/upgrade-plan-json.log" \
  node packages/cli/bin/run.js upgrade --plan --json --report-path "$PLAN_JSON"
...
artifact_dir = Path(".artifacts/foundry/upgrade-plan")
summary_txt = artifact_dir / "summary.txt"
summary_json = artifact_dir / "summary.json"
plan_json_path = Path(".artifacts/foundry/upgrade/plan.json")
...
text_status = read_status("upgrade plan text")
json_status = read_status("upgrade plan json")
...
report = {
    ...
    "checks": {
        ...
        "upgradePlanText": text_status,
        "upgradePlanJson": json_status,
    },
    "plan": {
    ...
    "logs": {
        ...
        "upgradePlanText": str(artifact_dir / "upgrade-plan-text.log"),
        "upgradePlanJson": str(artifact_dir / "upgrade-plan-json.log"),
    },
    "notes": [],
}

if not report["ok"]:
    report["notes"].append("Upgrade plan smoke gate failed. Inspect logs and plan JSON.")

summary_json.write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")

print("")
print("Foundry upgrade plan smoke summary")
print("==================================")
print("ok:", report["ok"])
...
print("")
print("Wrote:")
print("- .artifacts/foundry/upgrade-plan/summary.txt")
print("- .artifacts/foundry/upgrade-plan/summary.json")
