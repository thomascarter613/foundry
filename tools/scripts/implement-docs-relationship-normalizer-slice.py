...
FILES: dict[str, str] = {
    "packages/cli/src/docs/relationship-normalizer.ts": '''import { writeFileSync } from "node:fs";
...
''',
    ...
    "packages/cli/src/commands/docs/relationships/normalize.ts": '''import { Command, Flags } from "@oclif/core";
...
def update_package_json() -> None:
    ...
    scripts.setdefault(
        "docs:relationships:normalize",
        "bun run foundry:build && node packages/cli/bin/run.js docs relationships normalize"
    )
    scripts.setdefault(
        "docs:relationships:write",
        "bun run foundry:build && node packages/cli/bin/run.js docs relationships normalize --write"
    )
